...
const acceptFriendRequest = (id,idx) => {
  ...
  // console.log(id)
  qq("verifyFriend",{friend_id: id}).then(res=>{
    if(res.code == 200){
...
const handleSearch = () => {
  if (searchQuery.value.trim()) {
    ...
    qq("searchFriend",{username: searchQuery.value}).then(res=>{
      if(res.code == 200){
        console.log(res.data)
        searchResultsFriend.value = res.data.result;
      }
    });
    qq("searchGroup",{group_name: searchQuery.value}).then(res=>{
      if(res.code == 200){
        console.log(res.data)
...
const acceptGroupRequest = (user_id,group_id,idx) => {
  ...
  qq("verifyGroup",{group_id: group_id,user_id: user_id}).then(res=>{
    if(res.code == 200){
      console.log(res.data)
      notices.value.splice(idx,1)
    }
...
const handleAddAction = () => {
  ...
  if (modalInput.value.trim()) {
    if (currentAction.value === 'addFriend') {
      ...
      qq("addFriend",{friend_id: parseInt(modalInput.value)}).then(res=>{
        if(res.code == 200){
      ...
      qq("addGroup",{group_id: parseInt(modalInput.value)}).then(res=>{
        if(res.code == 200){
          groups.value.push(res.data)
        }
      })
...
const markAsRead = (data) => {
  ...
  qq("markRead",{message_id: data.message_id}).then(res=>{
    if(res.code == 200){
      // console.log(res.data)
    }});
}
...
const deleteFriend = () => {
  ...
  if (curChat.value && curChat.value.message_type === 'private') {
    const friend_id = curChat.value.sender_id == user.value.id ? curChat.value.receiver_id : curChat.value.sender_id;
    qq("deleteFriend",{friend_id: friend_id}).then(res=>{
      if(res.code == 200){
...
const openMemberManagement = () => {
  ...
  if (curChat.value && curChat.value.message_type === 'group' && (curChat.value.user_role === 'admin' || curChat.value.user_role === 'owner')) {
    qq("getGroup",{group_id: curChat.value.group_id}).then(res=>{
      if(res.code == 200){
...
const exitGroup = () => {
  if (curChat.value && curChat.value.message_type === 'group' && curChat.value.user_role !== 'owner') {
    qq("exitGroup",{group_id: curChat.value.group_id}).then(res=>{
      if(res.code == 200){
...
const deleteGroup = () => {
  if (curChat.value && curChat.value.message_type === 'group' && curChat.value.user_role === 'owner') {
    qq("deleteGroup",{group_id: curChat.value.group_id}).then(res=>{
      if(res.code == 200){
        groups.value = groups.value.filter(group => group.id !== curChat.value.group_id)
      }
    })
...
const changeMemberRole = (member) => {
  ...
  qq("modifyPermission",{group_id: curChat.value.group_id,user_id: member.id, role: member.role == 'member' ? 'admin' : 'member'
  }).then(res=>{
    if(res.code == 200){
...
const removeMember = (member) => { // TODO:
  qq("kickGroupmember",{group_id: curChat.value.group_id,user_id: member.id}).then(res=>{
    if(res.code == 200){
...
const updateUserProfile = () => {
  ...
  qq("updateProfile",editingUserProfile.value).then(res=>{
    if(res.code == 200){
